...
export function checkPolicy(
  ...
  if (state.notionalTodayUsd + trade.notionalUsd > cfg.maxDailyNotionalUsd) {
    return { ok: false, why: "exceeds maxDailyNotionalUsd" };
  }
  if (state.tradesToday + 1 > cfg.maxTradesPerDay) {
    return { ok: false, why: "exceeds maxTradesPerDay" };
  }

  if (state.lastTradeAtMs) {
    const mins = (nowMs - state.lastTradeAtMs) / 60000;
    if (mins < cfg.cooldownMinutes) {
      return { ok: false, why: `cooldown ${mins.toFixed(1)}m < ${cfg.cooldownMinutes}m` };
    }
  }
